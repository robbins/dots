  ...
  in mkIf cfg.enable {
    services.swayidle = {
      ...
      timeouts = [
        ...
        { timeout = 120; command = "${isOnBattery} && ${dim-screen}"; resumeCommand = "${isOnBattery} && ${undim-screen}"; } # If on battery dim screen after 2 minutes
        { timeout = 300; command = lock; } # Lock screen after 5 minutes
        { timeout = 360; command = dpms-off; } # Turn off display after 6 minutes
        { timeout = 600; command = "${isOnBattery} && ${suspend}"; } # If on battery suspend after 10 minutes
      ];
    };
  };
}

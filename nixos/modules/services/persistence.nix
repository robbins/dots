  ...
  in lib.mkIf cfg.enable {
    ...
    environment.persistence."${cfg.persistenceRoot}" = {
      ...
      users."${specialArgs.username}" = {
        ...
        ++ (if (homeCfg.modules.dev.android.enable or false) then [ ".config/gradle" ".config/android" ".config/Google" ".cache/Google" ] else []) # TODO: if system adb module as well? maybe if androidFeature where androidFeature is a global of home.android or system.android?
